...
const Dashboard = ({ 
    pulseData, 
    contingencyPotMinimum,
    contingencyPotBalance,
    onOverridePulse, 
    partnerNames, 
    capitalShares, 
    laborShares, 
    transactions, 
    charityPercentage, 
    capitalPoolPercentage = 0.50,
    labourPoolPercentage = 0.50,
    partnershipMode,
    labourShareMode,
    currencySymbol,
    partnerShares, 
    ...
    // Dual-Pool Math
    // Emulate contingency pot allocation before determining active profit pool bounds
    let validNetProfit = pulseData.netProfit;
    let realtimeContingencyAllocation = 0;

    if (validNetProfit > 0) {
        const shortfall = Math.max(0, (contingencyPotMinimum || 10000) - (contingencyPotBalance || 0));
        realtimeContingencyAllocation = Math.min(validNetProfit, shortfall);
        validNetProfit -= realtimeContingencyAllocation;
    }
    
    // Determine pool sizes based on partnership mode
    let capitalPoolMultiplier = capitalPoolPercentage;
    let labourPoolMultiplier = labourPoolPercentage;
    ...
    let capitalPoolSize = 0;
    let labourPoolSize = 0;
    let capitalCharityDeduction = 0;
    let labourCharityDeduction = 0;

    if (validNetProfit < 0) {
        // Losses are absorbed entirely by the capital pool
        capitalPoolSize = validNetProfit;
        labourPoolSize = 0;
        capitalCharityDeduction = 0;
        labourCharityDeduction = 0;
    } else {
        capitalPoolSize = validNetProfit * capitalPoolMultiplier;
        labourPoolSize = validNetProfit * labourPoolMultiplier;
        capitalCharityDeduction = capitalPoolSize * charityPercentage;
        labourCharityDeduction = labourPoolSize * charityPercentage;
    }
    ...
    const formatCurrency = (val) => {
        ...
        return numVal < 0 ? `-${currencySymbol}${formatted}` : `${currencySymbol}${formatted}`;
    };
    ...
    const partnersData = partnerNames.map((name, i) => {
        ...
        const volDeduction = grossPayout > 0 ? grossPayout * volPct : 0;
    ...
    const generatePDF = (report) => {
        ...
        const summaryRows = [
            ["Gross Net Profit", formatCurrency(report.net_profit)],
            ["Contingency Pot Deduction", `-${formatCurrency(data.contingency_pot_allocation || 0)}`],
            ["Distributable Profit", formatCurrency(data.distributable_net_profit !== undefined ? data.distributable_net_profit : report.net_profit)],
            ["Capital Pool Percentage", `${((data.capital_pool_percentage || 0.5) * 100).toFixed(1)}%`],
            ["Labour Pool Percentage", `${((data.labour_pool_percentage || 0.5) * 100).toFixed(1)}%`],
            [`Global Charity Allocation (${(charityPercentage * 100).toFixed(1)}%)`, formatCurrency(report.global_charity)],
        ...
        if (partnershipMode !== 'capital') columns.push('Labour');
    ...
    return (
        <div className="flex-column" style={{ gap: '2.5rem' }}>
            ...
            <header className="animate-fade-in glass-card">
                ...
                <div className="grid md:grid-cols-4 gap-6">
                    <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                            ...
                            <input
                                type="number"
                                step="0.01"
                                value={typeof pulseData.revenue === 'number' ? pulseData.revenue.toFixed(2) : pulseData.revenue}
                    ...
                    <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                            ...
                            <input
                                type="number"
                                step="0.01"
                                value={typeof pulseData.expenses === 'number' ? pulseData.expenses.toFixed(2) : pulseData.expenses}
                    ...
                    <div>
                        ...
                        <p className="text-xl fw-bold text-secondary">{formatCurrency(validNetProfit)}</p>
                        {realtimeContingencyAllocation > 0 && <p className="text-[10px] text-muted mt-1">After {formatCurrency(realtimeContingencyAllocation)} reserve minimum</p>}
                    </div>
                    <div>
                        <p className="text-muted mb-2 text-accent">Retained Contingency Cash</p>
                        <p className="text-xl fw-bold" style={{ color: '#10b981' }}>{formatCurrency(contingencyPotBalance || 0)}</p>
                    </div>
                </div>
            </header>
            ...
            <section className="animate-fade-in delay-100">
                <div className="flex-between mb-4">
                    <h2 className="m-0">{pulseData.netProfit < 0 ? "Loss Designation" : "Profit Designation"}</h2>
                    {realtimeContingencyAllocation > 0 ? (
                        <span className="fw-bold" style={{ fontSize: '0.9rem', color: '#38bdf8', background: 'rgba(56, 189, 248, 0.1)', padding: '0.4rem 0.8rem', borderRadius: '1rem', border: '1px solid rgba(56, 189, 248, 0.3)' }}>
                           Contingency Held: -{formatCurrency(realtimeContingencyAllocation)}
                        </span>
                    ) : (contingencyPotBalance >= contingencyPotMinimum && contingencyPotMinimum > 0) ? (
                        <span className="fw-bold" style={{ fontSize: '0.9rem', color: '#10b981', background: 'rgba(16, 185, 129, 0.1)', padding: '0.4rem 0.8rem', borderRadius: '1rem', border: '1px solid rgba(16, 185, 129, 0.3)' }}>
                           Reserve Fully Funded! No deduction needed.
                        </span>
                    ) : null}
                </div>
                <div className="grid md:grid-cols-2">
                    ...
                    <div className="glass-card flex-column" style={{ alignItems: 'center' }}>
                        <h3 className="mb-6">Capital Pool ({partnershipMode === 'both' ? `${(capitalPoolPercentage * 100).toFixed(1)}%` : '100%'})</h3>
                    ...
                    <div className="glass-card flex-column" style={{ alignItems: 'center' }}>
                        <h3 className="mb-6">Labour Pool ({partnershipMode === 'both' ? `${(labourPoolPercentage * 100).toFixed(1)}%` : '100%'})</h3>
            ...
            <section className="animate-fade-in delay-200 glass-card" style={{ overflowX: 'auto' }}>
                <h2 className="mb-4">{validNetProfit < 0 ? "Partner Loss Accrual" : "Partner Payout Accrual"}</h2>
                <table className="data-table">
                    <thead>
                        <tr>
                            <th>Partner</th>
                            {partnershipMode !== 'labour' && <th>Cap Share %</th>}
                            {partnershipMode !== 'labour' && <th>{validNetProfit < 0 ? "Cap Loss" : "Cap Amount"}</th>}
                            {partnershipMode !== 'capital' && <th>Lab Share %</th>}
                            {partnershipMode !== 'capital' && <th>{validNetProfit < 0 ? "Lab Loss" : "Lab Amount"}</th>}
                            <th>{validNetProfit < 0 ? "Gross Loss" : "Gross Payout"}</th>
                            <th>Voluntary Charity</th>
                            <th>{validNetProfit < 0 ? "Net Loss" : "Net Accrued"}</th>
                        </tr>
                    </thead>
                    <tbody>
            ...
            <CSVUpload token={token} onUploadSuccess={onRefreshTransactions} />
