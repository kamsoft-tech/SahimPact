...
function App() {
  ...
  const [capitalPoolPercentage, setCapitalPoolPercentage] = useState(0.50);
  const [labourPoolPercentage, setLabourPoolPercentage] = useState(0.50);
  ...
  const [contingencyPotMinimum, setContingencyPotMinimum] = useState(10000);
  const [contingencyPotBalance, setContingencyPotBalance] = useState(0);
  ...
  useEffect(() => {
    ...
    fetch('/api/settings', { headers })
      ...
      .then(data => {
        ...
        setCapitalPoolPercentage(data.capital_pool_percentage !== undefined ? data.capital_pool_percentage : 0.50);
        setLabourPoolPercentage(data.labour_pool_percentage !== undefined ? data.labour_pool_percentage : 0.50);
        ...
        setContingencyPotMinimum(data.contingency_pot_minimum !== undefined ? data.contingency_pot_minimum : 10000);
      })
    ...
    // Fetch Contingency Balance
    fetch('/api/distribution/contingency-balance', { headers })
      .then(res => res.json())
      .then(data => setContingencyPotBalance(data.balance || 0))
      .catch(err => console.error("Error fetching pot balance:", err));
  ...
  const updateCharityPercentage = (val) => {
    ...
    fetch('/api/settings', {
      ...
      body: JSON.stringify({ 
        ...
        capital_pool_percentage: capitalPoolPercentage,
        labour_pool_percentage: labourPoolPercentage,
        contingency_pot_minimum: contingencyPotMinimum
      })
    }).catch(err => console.error("Error updating settings:", err));
  };

  const updatePoolPercentages = (capPoolPct, labPoolPct) => {
    setCapitalPoolPercentage(capPoolPct);
    setLabourPoolPercentage(labPoolPct);
    fetch('/api/settings', {
      method: 'PUT',
      headers: { 
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({ 
        ...
        currency_symbol: currencySymbol,
        is_setup_complete: isSetupComplete,
        capital_pool_percentage: capPoolPct,
        labour_pool_percentage: labPoolPct,
        contingency_pot_minimum: contingencyPotMinimum
      })
    }).catch(err => console.error("Error updating pool settings:", err));
  };
  ...
  const updateContingencyPotMinimum = (val) => {
    setContingencyPotMinimum(val);
    fetch('/api/settings', {
      method: 'PUT',
      headers: { 
        'Content-Type': 'application/json',
  ...
  const handleBulkDeleteTransactions = async (txIds) => {
    if (!window.confirm(`Are you sure you want to delete ${txIds.length} transaction(s)?`)) return;
    try {
    ...
    <div style={{ 
      ...
      <aside style={{
        ...
        <Sidebar 
          ...
          capitalPoolPercentage={capitalPoolPercentage}
          labourPoolPercentage={labourPoolPercentage}
          onUpdatePoolPercentages={updatePoolPercentages}
          partnershipMode={partnershipMode}
          labourShareMode={labourShareMode}
          currencySymbol={currencySymbol}
          onUpdateSystemModes={updateSystemModes}
          contingencyPotMinimum={contingencyPotMinimum}
          contingencyPotBalance={contingencyPotBalance}
          onUpdateContingency={updateContingencyPotMinimum}
